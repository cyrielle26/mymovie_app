...
// export const discover = async (type, genre) => {
// 	let discoverUrl = `${baseUrl}discover/${type}?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=${genre}`
// 	return fetch(discoverUrl, options).then((res) => res.json())
// }

export const discover = async (type, genre, currentPage) => {
	// Base URL
	let discoverUrl = `${baseUrl}discover/${type}?include_adult=false&include_video=false&language=en-US&page=${currentPage}&sort_by=popularity.desc`

	// If genre is provided, add it to the URL
	if (genre) {
		discoverUrl += `&with_genres=${genre}`
	}

	// Fetch request
	return fetch(discoverUrl, options)
		.then((res) => res.json())
		.catch((error) => {
			// Handle errors
